...
def store(request, category_slug=None):
    ...
    return render(request, 'store/store.html', {'category': category_page, 'products': products})
...
#Check if cart exitst by checking session id.
#If no session id, then create a new cart
#Check CartItem to see if item exits. If so add to it
#If no item exits, then create a new on.
...
def cart_detail(request, total=0, counter=0, cart_items=None):
    ...
    description = 'Playing Card Store -New Order'
